...
if [ "${INPUT_VERSION}" = "latest" ]; then
  ...
  VERSION=$(curl -s https://api.github.com/repos/dobbo-ca/editorlint/releases/latest | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
  if [ -z "$VERSION" ]; then
...
if [ "$GOOS" = "windows" ]; then
  ...
  ARCHIVE_NAME="editorlint-${VERSION}-${GOOS}-${GOARCH}.zip"
else
  BINARY_NAME="editorlint"
  ARCHIVE_NAME="editorlint-${VERSION}-${GOOS}-${GOARCH}.tar.gz"
fi

DOWNLOAD_URL="https://github.com/dobbo-ca/editorlint/releases/download/${VERSION}/${ARCHIVE_NAME}"
